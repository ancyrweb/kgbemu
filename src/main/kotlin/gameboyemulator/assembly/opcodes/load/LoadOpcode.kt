    ...
    fun fromBytes(bytes: ByteArray, index: Int, address: Int): LoadOpcode? {
      ...
      return when (opcode) {
        ...
        0x06 -> {
          ...
          LoadOpcode(address, RegisterSource("B"), ImmediateValueSource(value, ImmediateValueSource.Size.BYTE), bytesSize = 2, highClockCycle = 8, lowClockCycle = 8)
        }
        0x0E -> {
          val value = bytes[index + 1].toInt() and 0xFF
          LoadOpcode(address, RegisterSource("C"), ImmediateValueSource(value, ImmediateValueSource.Size.BYTE), bytesSize = 2, highClockCycle = 8, lowClockCycle = 8)
        }
        0x16 -> {
          val value = bytes[index + 1].toInt() and 0xFF
          LoadOpcode(address, RegisterSource("D"), ImmediateValueSource(value, ImmediateValueSource.Size.BYTE), bytesSize = 2, highClockCycle = 8, lowClockCycle = 8)
        }
        0x1E -> {
          val value = bytes[index + 1].toInt() and 0xFF
          LoadOpcode(address, RegisterSource("E"), ImmediateValueSource(value, ImmediateValueSource.Size.BYTE), bytesSize = 2, highClockCycle = 8, lowClockCycle = 8)
        }
        0x26 -> {
          val value = bytes[index + 1].toInt() and 0xFF
          LoadOpcode(address, RegisterSource("H"), ImmediateValueSource(value, ImmediateValueSource.Size.BYTE), bytesSize = 2, highClockCycle = 8, lowClockCycle = 8)
        }
        0x2E -> {
          val value = bytes[index + 1].toInt() and 0xFF
          LoadOpcode(address, RegisterSource("L"), ImmediateValueSource(value, ImmediateValueSource.Size.BYTE), bytesSize = 2, highClockCycle = 8, lowClockCycle = 8)
        }
        0x3E -> {
          val value = bytes[index + 1].toInt() and 0xFF
          LoadOpcode(address, RegisterSource("A"), ImmediateValueSource(value, ImmediateValueSource.Size.BYTE), bytesSize = 2, highClockCycle = 8, lowClockCycle = 8)
        }
        ...
        0x36 -> {
          val value = bytes[index + 1].toInt() and 0xFF
          LoadOpcode(address, HLAddressSource(), ImmediateValueSource(value, ImmediateValueSource.Size.BYTE), bytesSize = 2, highClockCycle = 12, lowClockCycle = 12)
        }
        ...
        0xFA -> {
          ...
          LoadOpcode(address, RegisterSource("A"), MemoryAddressSource(addr), bytesSize = 3, highClockCycle = 16, lowClockCycle = 16)
        }
        ...
        0xEA -> {
          ...
          val high = bytes[index + 2].toInt() and 0xFF
          val addr = (high shl 8) or low
          LoadOpcode(address, MemoryAddressSource(addr), RegisterSource("A"), bytesSize = 3, highClockCycle = 16, lowClockCycle = 16)
        }
        ...
        0xF0 -> {
          ...
          LoadOpcode(address, RegisterSource("A"), IOPortImmediateSource(offset), bytesSize = 2, highClockCycle = 12, lowClockCycle = 12)
        }

        // LDH (n), A - Load from A to memory address 0xFF00+n
        0xE0 -> {
          val offset = bytes[index + 1].toInt() and 0xFF
          LoadOpcode(address, IOPortImmediateSource(offset), RegisterSource("A"), bytesSize = 2, highClockCycle = 12, lowClockCycle = 12)
        }
        ...
        0x01 -> {
          ...
          LoadOpcode(address, RegisterSource("BC"), ImmediateValueSource(value, ImmediateValueSource.Size.WORD), bytesSize = 3, highClockCycle = 12, lowClockCycle = 12)
        }
        0x11 -> {
          val low = bytes[index + 1].toInt() and 0xFF
          val high = bytes[index + 2].toInt() and 0xFF
          val value = (high shl 8) or low
          LoadOpcode(address, RegisterSource("DE"), ImmediateValueSource(value, ImmediateValueSource.Size.WORD), bytesSize = 3, highClockCycle = 12, lowClockCycle = 12)
        }
        0x21 -> {
          val low = bytes[index + 1].toInt() and 0xFF
          val high = bytes[index + 2].toInt() and 0xFF
          val value = (high shl 8) or low
          LoadOpcode(address, RegisterSource("HL"), ImmediateValueSource(value, ImmediateValueSource.Size.WORD), bytesSize = 3, highClockCycle = 12, lowClockCycle = 12)
        }
        0x31 -> {
          val low = bytes[index + 1].toInt() and 0xFF
          val high = bytes[index + 2].toInt() and 0xFF
          val value = (high shl 8) or low
          LoadOpcode(address, RegisterSource("SP"), ImmediateValueSource(value, ImmediateValueSource.Size.WORD), bytesSize = 3, highClockCycle = 12, lowClockCycle = 12)
        }
        ...
        0x08 -> {
          ...
          val high = bytes[index + 2].toInt() and 0xFF
          val addr = (high shl 8) or low
          LoadOpcode(address, MemoryAddressSource(addr), RegisterSource("SP"), bytesSize = 3, highClockCycle = 20, lowClockCycle = 20)
        }

        // LDHL SP, n - Load SP + signed byte n into HL
        0xF8 -> {
          val offset = bytes[index + 1].toInt() // Signed byte
          LoadOpcode(address, RegisterSource("HL"), SPOffsetSource(offset), bytesSize = 2, highClockCycle = 12, lowClockCycle = 12)
        }
